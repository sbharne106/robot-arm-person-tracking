...
BASE_MAX_STEP = 2.5
SHOULDER_MAX_STEP = 0.8
ELBOW_MAX_STEP = 2.0

# Direction signs
# Base was reversed, so now it is -1
BASE_TRACK_SIGN = -1
...
def step_toward(current, target, max_step):
    ...
    This is what makes the motor movement smoother.
    """
    diff = target - current
...
try:
    while True:
        ...
        if person_box is not None:
            ...
            # ELBOW TRACKING
            # ------------------------------
            # Old version only used size_error, so elbow barely moved.
            # New version uses both person size and vertical position.
        ...
        # Each motor now has its own smoothness value.
        # ====================================================
