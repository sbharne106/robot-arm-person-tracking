...
# ============================================================
# LED MATRIX ESP32 SETTINGS
# ============================================================
# Change this to the ESP32 IP address for the LED matrix.
# This is separate from any ESP32/microcontroller used for motors.
LED_ESP32_IP = "192.168.21.158"

LED_FRAME_URL = f"http://{LED_ESP32_IP}/frame"
LED_CLEAR_URL = f"http://{LED_ESP32_IP}/clear"
LED_BRIGHTNESS_URL = f"http://{LED_ESP32_IP}/brightness"

LED_BRIGHTNESS = 35
LED_REQUEST_TIMEOUT = 2

ANIMATION_DELAY = 0.07
EMOJI_HOLD_TIME = 0.35
EMOJI_CLEAR_TIME = 0.12
...
# These use your confirmed physical LED indexes.
EMOJI_PRESETS = {
    "smiley face": {
        "color": YELLOW,
        "pixels": [
            23, 29, 35, 41, 16, 28, 34, 46,
            9, 15, 21, 27, 33, 39, 45, 51,
            8, 14, 26, 32, 44, 50,
            13, 19, 37, 43, 18, 24, 30, 36
        ],
        "extra": {
            BLUE: [22, 40, 20, 38, 25, 31]
        }
    },
    ...
    "butterfly": {
        "color": MAGENTA,
        "pixels": [
            41, 40, 33, 39, 14, 20, 26, 32, 38, 44,
            19, 25, 31, 37, 43, 24, 30
        ],
        "extra": {}
    },
    ...
    "right arrow": {
        "color": RED,
        "pixels": [
            47, 52, 3, 9, 15, 21, 27, 33, 39, 45,
            51, 57, 2, 8, 14, 20, 26, 32, 38, 44,
            50, 56, 49, 42
        ],
        "extra": {}
    },
    ...
    "sideways arrow": {
        "color": BLUE,
        "pixels": [41, 47, 53, 59, 52, 58, 45, 57, 38, 56, 31, 24],
        "extra": {}
    },
    ...
    "star": {
        "color": BLUE,
        "pixels": [
            35, 28, 34, 15, 21, 27, 33, 39, 45,
            20, 26, 32, 38, 19, 25, 31, 18, 30, 36
        ],
        "extra": {}
    },
    ...
    "up arrow": {
        "color": RED,
        "pixels": [29, 35, 22, 28, 34, 40, 15, 27, 33, 45, 26, 32, 25, 31, 24, 30],
        "extra": {}
    },

    "down arrow": {
        "color": RED,
        "pixels": [29, 35, 28, 34, 27, 33, 14, 26, 32, 44, 19, 25, 31, 37, 24, 30],
        "extra": {}
    },
...
led_current_emoji = "Starting"
led_connection_ok = False
...
def led_pixels_to_hex_string(physical_pixels):
    output = ""
...
def send_led_pixels_to_esp32(physical_pixels):
    """
    Sends one complete 60-LED frame to the LED-matrix ESP32.
    Errors are caught so LED Wi-Fi issues do not stop the robot arm.
    """
    global led_connection_ok

    hex_data = led_pixels_to_hex_string(physical_pixels)
    ...
    try:
        response = requests.post(
            ...
            timeout=LED_REQUEST_TIMEOUT
        )

        led_connection_ok = True
        return response

    except Exception as error:
        led_connection_ok = False
        print(f"LED ESP32 frame send failed: {error}")
        return None


def clear_led_matrix():
    """
    Clears the physical LED matrix.
    """
    global led_connection_ok
    ...
    try:
        ...
        led_connection_ok = False
        print(f"LED ESP32 clear failed: {error}")
        return None


def set_led_brightness(value):
    """
    Sets LED brightness on the ESP32.
    """
    global led_connection_ok
    ...
    try:
        ...
        led_connection_ok = False
        print(f"LED ESP32 brightness failed: {error}")
        return None


def get_preset_animation_steps(preset_name):
    """
    Returns the LED indexes and colors in the order they should light up.
    """
    preset = EMOJI_PRESETS.get(preset_name)
    ...
    main_color = preset["color"]

    for index in preset["pixels"]:
        if 0 <= index < NUM_LEDS:
            steps.append((index, main_color))

    for color, indexes in preset.get("extra", {}).items():
        for index in indexes:
            if 0 <= index < NUM_LEDS:
                steps.append((index, color))
...
def led_emoji_loop():
    ...
    smiley -> butterfly -> arrows -> star -> heart -> repeat.
    """
    global led_current_emoji

    print("Starting LED emoji loop...")

    set_led_brightness(LED_BRIGHTNESS)
    clear_led_matrix()

    while not shutdown_event.is_set():
        for preset_name in EMOJI_SEQUENCE:
            if shutdown_event.is_set():
                break

            with led_lock:
                led_current_emoji = preset_name

            steps = get_preset_animation_steps(preset_name)
            physical_pixels = [(0, 0, 0) for _ in range(NUM_LEDS)]

            for index, color in steps:
                if shutdown_event.is_set():
                    break

                physical_pixels[index] = color
                send_led_pixels_to_esp32(physical_pixels)
                time.sleep(ANIMATION_DELAY)

            time.sleep(EMOJI_HOLD_TIME)

            clear_led_matrix()
            time.sleep(EMOJI_CLEAR_TIME)

    with led_lock:
        led_current_emoji = "Stopped"

    clear_led_matrix()
    print("LED emoji loop stopped.")
# ============================================================
...
def status():
    ...
    with state_lock:
        return jsonify({
            ...
            "t1_active": t1_active,
            "led_current_emoji": led_current_emoji,
            "led_connection_ok": led_connection_ok
        })
