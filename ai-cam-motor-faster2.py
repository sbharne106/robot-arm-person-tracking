...
# Higher gain = target changes faster
BASE_GAIN = 5.0
SHOULDER_GAIN = 6.0
ELBOW_GAIN = 8.0
...
def write_all_motors(base_offset, shoulder_offset, elbow_offset):
    """
    Sends updated angles to all 3 motors.
    """
...
def disable_all_motors():
    """
    Cuts off servo signal after motors are safely back at mean.
    """
...
def go_to_mean_position():
    """
    Smoothly returns all motors to mean offset 0 before shutdown.
    """
...
print("Camera opened.")
print("Press q to quit. Motors will return to mean position and then cut off.")
...
try:
    while True:
        ...
        if person_box is not None:
            ...
            # Clamp targets to safe mechanical range
        ...
        # ====================================================
        # FAST COORDINATED MOTOR MOVEMENT
        # ====================================================
        ...
        # T1 safety applied to actual movement
        current_shoulder_offset, current_elbow_offset = apply_t1_safety(
            current_shoulder_offset,
            current_elbow_offset
        )
        ...
        cv2.imshow("AI 3 Motor Person Tracking - Fast", frame)
    ...
    # Then safely return motors to mean
    go_to_mean_position()
